...
int START = 0; //xy
int DEST_default[] = {15,20,23,44}; //MAX 10 destination
...
void findRouteUCS(int destPos, int routeUCS[]) {
  ...
  while (!isArrEmpty2D(frontier,0)) { //while frontier not empty
    ...
    for (int i=0; i<(SIZE+1)*(SIZE+1); i++) {
      if (frontier[i][0]!=-1) {
        Serial.print(frontier[i][0]);
        Serial.print(",");
        Serial.print(frontier[i][1]);
        Serial.print(" | ");
      }
    }
    Serial.println("");
...
/////////////////////////////////////////////////
//wifi and 2-car related function
void carNumSend() {
...
void obsDestUpdate2car() {
  ...
  if (Serial2.available()) {
    arrInitToSize(Obstacle); //init to obs list
...
void setupWizardOLED() {
    ...
    } else if (Car==2) { //car 2 is remote, only pos can set
      setupWizard_pos(tDelay);
    }
